...
| --- | ---: | ---: |
| Точность, `1 − nMAE` | **85,0%** | **84,8%** |
| Средняя ошибка (nMAE), доля номинала | 15,0% | 15,2% |
| Часов, где ошибка не больше ±10% номинала | 54,0% | 54,2% |
| nMAE на горизонте 1–24 ч / 25–48 ч | 14,6% / 15,4% | 14,9% / 15,5% |
| Ошибка суточной выработки | 9,8% | 9,9% |
| Лучше эмпирической кривой мощности | 20,8% | 20,8% |
| Лучше климатологии (среднее по часу суток) | 53,7% | 52,6% |
| Пересчёт через +6 / +12 / +18 ч снижает ошибку на те же часы | 2,7% / 4,3% / 5,9% | 2,4% / 4,0% / 5,8% |
| 80%-ный интервал на проверке покрыл фактов | 83,2% | 83,7% |
...
- обучение с проверкой — около 4 минут на турбину;
...
| --- | --- |
| Модель на исторических данных | LightGBM на признаках погоды (среднее трёх seed), выбран по проверке из четырёх вариантов; обучение с 16.02.2024 по 31.01.2026 (`train.py`) |
...
flowchart LR
    ...
    subgraph A["ForecastAgent"]
        ...
        V1 --> M["run_forecast_model<br/>LightGBM, среднее трёх seed"]
...
2. **Данные и модель.** `src/features.py` строит 36 признаков: основной ряд ICON (ветер, температура, порывы, направление, давление, влажность), плотность воздуха, ветер на 100 м отдельно по ECMWF IFS 0,25°, ECMWF IFS 9 км, нейросетевой ECMWF AIFS и GFS, ветер на 10 м по UK Met Office, CMA и JMA, разброс между моделями, порывы, сдвиг ветра, направление, давление, влажность, горизонт и время суток. Час SCADA — это среднее за 50 минут, а прогноз погоды мгновенный, поэтому значения погоды усредняются по началу и концу часа. Вариант модели выбирается по наименьшей средней ошибке на проверке среди четырёх: кривая мощности, кривая с поправкой LightGBM, прямой LightGBM и его среднее с каскадом (по тем же признакам предсказывается ветер на анемометре турбины, затем мощность по кривой, построенной по трём годам SCADA). Выбран прямой LightGBM: каскад на проверке не дал выигрыша. Модель — среднее трёх LightGBM с разными seed. Две квантильные модели задают 80%-ный интервал.
...
- **Восемь погодных моделей как признаки.** Там, где модели расходятся, прогноз ненадёжен. Больше всего дала нейросетевая ECMWF AIFS: она точнее остальных на этой площадке.
- **Одна погодная модель в основе на всю историю.** Смешанный ряд Open-Meteo `best_match` для этой точки до 30.09.2025 совпадает с ICON, а с 01.10.2025 — с ECMWF IFS 9 км: в январе 2026 средний ветер в нём на 1,7 м/с ниже, чем у ICON. Модель, обученная на нём, училась бы на одной погодной модели, а прогнозировала бы февраль по другой. Поэтому основной ряд — ICON за весь период, а IFS 9 км — отдельный признак.- **Калиброванная неопределённость.** Ширина интервала подбирается на отдельном месяце, покрытие проверяется на следующем.
...
- **Погода**: `data/cache/previous_runs_v4_*.csv.gz`. Основной ряд ICON содержит ветер на 100 м, температуру, ветер на 10 м, порывы, направление, давление и влажность. Отдельно лежат ветер на 100 м по ECMWF IFS 0,25°, ECMWF IFS 9 км (с 01.10.2025), ECMWF AIFS (с 18.02.2025) и GFS и ветер на 10 м по UK Met Office, CMA и JMA. Рядом в JSON записаны endpoint, узлы сетки, время запроса и sha256 ответа. Архив `previous_dayN` на 100 м доступен с 16.02.2024, поэтому обучение начинается с этой даты; там, где у модели ещё нет архива, признак пустой, и LightGBM это учитывает.
...
| Open-Meteo Previous Runs API | Архивные прогнозы погоды ICON, ECMWF IFS (0,25° и 9 км), ECMWF AIFS, GFS, UK Met Office, CMA, JMA |
